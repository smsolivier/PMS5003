# PMS5003 
This library wraps a `HardwareSerial` or `SoftwareSerial` stream corresponding to a PMS5003's TX and RX pins. The library facilitates changing the sensor's operating modes (active vs. passive) and waking/sleeping the sensor. 
...
Example usage of this library with `SoftwareSerial`: 
``` 
...
void loop() {
	...
	// sleep the sensor 
	// tries holds the number of read attempts. data.valid == true means the data was correctly read 
	// data.mask holds debugging info and will indicate where the read failed. data.mask == 15 indicates 
	...
	String s = String("1.0: ") + String(data.pms.pm_st[0]) + String(", 2.5: ") + String(data.pms.pm_st[1]) + String(", 10.0: ") + String(data.pms.pm_st[2]); 
	if (!data.valid) {
